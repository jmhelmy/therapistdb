...
async function getTherapistData(slug: string) {
  return prisma.therapist.findUnique({
    where: { slug },
    select: {
      id: true, userId: true, name: true, slug: true, tagline: true, imageUrl: true, phone: true,
      ...
      educationSchool: true, educationDegree: true, educationYearGraduated: true,
      practiceStartYear: true, feeIndividual: true, feeCouples: true, slidingScale: true,
      freeConsultation: true, paymentMethods: true, insuranceAccepted: true, feeComment: true,
      issues: true, topIssues: true, // Ensure 'topIssues' is in your Prisma schema and selected
      ...
      nearbyCity1: true, nearbyCity2: true, nearbyCity3: true,
...
export async function generateMetadata(
  ...
  ].filter(Boolean).map(k => k!.trim()).filter(k => k.length > 1).slice(0, 15);
...
<div className="bg-slate-50 min-h-screen print:bg-white"> {/* Or bg-gray-50, bg-slate-100 */}
<div className="max-w-4xl mx-auto px-4 py-6 sm:py-10">
  
          <nav aria-label="Breadcrumb" className="text-sm text-gray-600 mb-4 sm:mb-6 print:hidden">
            <ol className="list-none p-0 inline-flex flex-wrap items-center gap-x-1 gap-y-0.5">
              <li className="flex items-center">
                ...
                <Link href="/" className={`${accentHoverColor} hover:underline`}>Home</Link>
              </li>
              <li className="flex items-center">
                <ChevronRight size={18} className="text-gray-400 mx-0.5" />
                <Users size={16} className="mr-1.5 text-gray-500" />
                <Link href="/therapists" className={`${accentHoverColor} hover:underline`}>Therapists</Link>
              </li>
              {therapist.primaryState && (
                <li className="flex items-center">
                  <ChevronRight size={18} className="text-gray-400 mx-0.5" />
                  <MapPinIcon size={16} className="mr-1.5 text-gray-500" />
                  <Link href={`/therapists?state=${slugify(therapist.primaryState)}`} className={`${accentHoverColor} hover:underline`}>
                    {therapist.primaryState}
                  </Link>
                </li>
              )}
              {therapist.primaryCity && therapist.primaryState && (
                 <li className="flex items-center">
                  <ChevronRight size={18} className="text-gray-400 mx-0.5" />
                  <Link href={`/therapists?city=${slugify(therapist.primaryCity)}&state=${slugify(therapist.primaryState)}`} className={`${accentHoverColor} hover:underline`}>
                    {therapist.primaryCity}
                  </Link>
                </li>
              )}
              <li className="flex items-center text-gray-800 font-medium" aria-current="page">
                <ChevronRight size={18} className="text-gray-400 mx-0.5" />

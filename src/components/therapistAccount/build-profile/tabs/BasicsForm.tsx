...
import {
  ...
  GENDER_OPTIONS,
} from "@/lib/schemas/therapistSchema";
...
export default function BasicsForm() {
  ...
  const figmaInputClassName =
    "bg-gray-100 border-gray-300 placeholder-gray-400 focus:border-teal-500 focus:ring-teal-500 text-sm";
  ...
  return (
    ...
    <div className="space-y-8">
      ...
      <div className="bg-[#F0EBE3] rounded-lg shadow-inner relative min-h-[240px] sm:min-h-[300px] md:min-h-[320px] flex flex-col justify-end items-center">
        ...
        */}
        <div className="relative z-10 mb-[-50px] sm:mb-[-60px]">
          <ImageUploadField
            name="imageUrl"
            label="Profile Photo"
      ...
      {(errors.coverImageUrl || errors.imageUrl) && (
        <div className="pt-[60px] sm:pt-[70px] text-center space-y-1">
          {" "}
          {/* Adjust based on actual overhang */}
          {errors.coverImageUrl && (
            <p className="text-xs text-red-500">
              Cover Image: {(errors.coverImageUrl as any)?.message}
            </p>
          )}
          {errors.imageUrl && (
            <p className="text-xs text-red-500">
              Profile Photo: {(errors.imageUrl as any)?.message}
            </p>
          )}
        </div>
      )}
      ...
      <div
        className={`bg-white p-4 sm:p-6 md:p-8 rounded-lg border border-gray-200 shadow-md space-y-4 sm:space-y-6 ${
          errors.coverImageUrl || errors.imageUrl
            ? "mt-4"
            : "mt-[70px] sm:mt-[80px]"
        }`}
      >
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mb-2">
            e.g. LMFT, PhD. These appear after your name.
          </p>
        ...
        <div>
          ...
          <div className="space-y-1 sm:space-y-2">
            {genderRadioOptionsForForm.map((option) => (
              <label
                key={option.value}
                className="flex items-center cursor-pointer group p-2 rounded-md hover:bg-gray-50 transition-colors"
              >
                <input
                  type="radio"
                ...
                />
                <span className="ml-2 sm:ml-2.5 text-sm text-gray-700 group-hover:text-gray-900">
                  {option.label}
                </span>
              </label>
            ))}

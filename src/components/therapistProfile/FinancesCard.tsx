...
export default function FinancesCard({ therapist }: FinancesCardProps) {
  ...
  const insuranceDisplay =
    insuranceAccepted && insuranceAccepted.trim() !== ""
      ? insuranceAccepted.trim()
      : null;

  const displayFee = (fee: string | null | undefined, sessionType: string) => {
    if (typeof fee !== "string" || fee.trim() === "") return null; // Safely check type before trim
    const feeValue =
      !fee.startsWith("$") && /^\d+(\.\d+)?$/.test(fee) ? `$${fee}` : fee;
    return (
      <p>
        <span className="font-medium">{sessionType}:</span> {feeValue}
      </p>
    );
  };
    ...
    >
      <div className="space-y-5 text-base">
        {(feeIndividual || feeCouples) && (
          <Section
            title="Session Fees"
            titleClassName="text-base font-semibold text-gray-700 mb-1.5"
          >
          ...
          >
            <div className="space-y-1.5">
              {slidingScale !== null && slidingScale !== undefined && (
                <p
                  className={`flex items-center ${
                    slidingScale ? "text-green-700" : "text-red-600"
                  }`}
                >
                  {slidingScale ? (
              ...
              {freeConsultation !== null && freeConsultation !== undefined && (
                <p
                  className={`flex items-center ${
                    freeConsultation ? "text-green-700" : "text-red-600"
                  }`}
                >
                  {freeConsultation ? (
                    <CheckCircle size={16} className="mr-2 shrink-0" />
                  ...
                  {freeConsultation ? "Offered" : "Not Offered"}
                </p>
              )}
            </div>
          </Section>
        ...
        {paymentsDisplay && (
          <Section
            ...
            icon={<CreditCard size={16} className="mr-1.5 text-gray-500" />}
            titleClassName="text-base font-semibold text-gray-700 mb-1.5"
          >
        ...
        {insuranceDisplay && (
          <Section
            title="Insurance"
            icon={<FileText size={16} className="mr-1.5 text-gray-500" />}
            titleClassName="text-base font-semibold text-gray-700 mb-1.5"
          >
            <p className="text-gray-700 whitespace-pre-wrap">
              {insuranceDisplay}
            </p>{" "}
            {/* pre-wrap for newlines in insurance string */}
            <p className="text-xs text-gray-500 mt-1.5">
              Always verify coverage directly with the therapist before your
              first session.
            </p>
          </Section>
        )}
        ...
        {feeComment && feeComment.trim() !== "" && (
          <Section
            title="Additional Fee Information"
            titleClassName="text-base font-semibold text-gray-700 mb-1.5"
          >
